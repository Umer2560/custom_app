...
// Custom Code
		...
		if (this.frm.doc.is_return) {
			...
			this.frm.toggle_reqd("due_date", false);
		}
		if (this.frm.doc.debit_note) {
			this.frm.return_print_format = "Sales Invoice Debit";
			this.frm.toggle_reqd("due_date", false);
		}
...
// Custom Code
			if(doc.outstanding_amount >= 0 || Math.abs(flt(doc.outstanding_amount)) < flt(doc.grand_total)) {
				cur_frm.add_custom_button(__('Debit Note'),
					this.make_sales_debit, __('Create'));
				cur_frm.page.set_inner_btn_group_as_primary(__('Create'));
			}
// Custom Code End
...
// Custom Function
	make_sales_debit() {
		frappe.model.open_mapped_doc({
			method: "custom_app.controllers.sales_debit.make_sales_debit",
			frm: cur_frm
		})
	}
// Custom Function End
